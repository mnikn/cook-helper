    ...
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="max-w-[800px] max-md:max-w-[90vw] flex flex-col">
        <div className="flex flex-col items-center">
          ...
          {/* 顶部大图 */}
          <div className={`w-full text-center mb-4 mt-6 ${items?.length === 0 ? "hidden" : ""}`}>
            <img
          ...
          {/* 底部缩略图预览 */}
          <div className={`flex gap-2 overflow-x-auto py-2 w-full overflow-x-auto ${items?.length === 0 ? "hidden" : ""}`}>
            {items?.map((item, idx) => (
              <div
                key={idx}
